...
#from DavesStarburstEyeFeatureFinder import *
...
class StarBurstEyeFeatureFinder (EyeFeatureFinder):
    ...
    def __init__(self):
        ...
        self.FlagSmooth = 0 
        self.mfact_grad_star_std = 2.5 #1.5
        self.Dpix_ToHigh = 3
        ...
        self.SB = StarBurstVectorized(self.mfact_grad_star_std, self.Dpix_ToHigh)
    ...
    def reinstantiate_starburst(self, mfact_grad_star_std = None, Dpix_ToHigh = None):
    ...
    def analyze_image(self, im, guess = None, **kwargs):
        ...
        # Pupil features
        if( guess != None ):
            if('pupil_position' in guess and guess['pupil_position'] is not None):
            ...
            if('pupil_radius_CV' in guess and guess['pupil_radius_CV'] is not None):
            ...
            if('dwnsmp_factor_coord' in guess and guess['dwnsmp_factor_coord'] is not None):
            ...
            if('dwnsmp_factor_size' in guess and guess['dwnsmp_factor_size'] is not None):
        ...
        if(self.starburst_object_parameters_updated):
            self.reinstantiate_starburst(self.mfact_grad_star_std, self.Dpix_ToHigh)
            self.starburst_object_parameters_updated = 0

        # ****************** Find CR center ******************
        # Compute the default ray length as a fraction of the image size
        self.RayLenght_default_CR = int( self.Perc_CRImgSize2RayLengDefault * self.im_array_whole.shape[1] )
        if self.FlagPrintCR:
            print 'percentage of the image taken to compute the CR ray length =', self.Perc_CRImgSize2RayLengDefault
        ...
        try:
            ...
            cr_info = self._find_CR( self.Nrays_quadCR, StartCRCoord, StartCRRadius, FlagForceGuessCRCntr, self.FlagPlotCR, self.FlagPrintCR )
        except Exception, e:
            print e.message
            if self.FlagCompAltGuesses:
                if size(StartCRCoord) != 0:
                    try:
                        ...
                        if self.FlagPlotFinal: print '------->>> Local search (threshold) on an image patch to find an intial guess for the CR'
                        self._clear()
                        FlagForceGuessCRCntr = True
                        cr_info = self._find_CR( self.Nrays_quadCR, StartCRCoord, StartCRRadius, FlagForceGuessCRCntr, self.FlagPlotCR, self.FlagPrintCR )
                    except:
                        # Finally, start fresh and look for an initial guess over the whole image
                        if self.FlagPlotFinal: print '------->>> Search (threshold) the whole image to find an intial guess for the CR'
                        self._clear()
                        StartCRCoord = []
                        cr_info = self._find_CR( self.Nrays_quadCR, StartCRCoord, StartCRRadius, FlagForceGuessCRCntr, self.FlagPlotCR, self.FlagPrintCR )
                ...
                # The initial guess based on the Th was wrong ... try other spots 
                    ...
                    I = range(len(iCR)); random.shuffle(I)  
    #                 iCR[I[0]] = 64
    #                 jCR[I[0]] = 120
                    k = 0
                    MaxCount = 10
                    ...
                    while FlagIterate and k < MaxCount:
                        ...
                        k = k+1
                        try:
                            if self.FlagPlotFinal: print '--->>> Attempt #', k, ': try as a new CR center guess: ', StartCRCoord
                            if FlagPlot:
                                # Plot image
                                figure()
                                ...
                                # Plot pupil
                                plot( jCR, iCR, 'r.' )
                                ho = plot( array([StartCRCoord[0]]), array([StartCRCoord[1]]), '+b' )
                                setp( ho, 'markersize', 10, 'markeredgewidth', 2 )
                                setp(gca(), 'xlim', [1, self.im_array_whole.shape[1]], 'ylim', [self.im_array_whole.shape[0], 1])
                            cr_info = self._findCR( self.Nrays_quadCR, StartCRCoord, StartCRRadius, FlagForceGuessCRCntr, self.FlagPlotCR, self.FlagPrintCR )
                        except:
                            FlagIterate = 1
        ...
        # ****************** Find Pupil center ******************
        # Compute the default ray length as a fraction of the image size
        self.RayLenght_default_Pup = int( self.Perc_PupImgSize2RayLengDefault * self.im_array_whole.shape[1] )
        if self.FlagPrintPupil:
            print 'percentage of the image taken to compute the Pupil ray length =', self.Perc_PupImgSize2RayLengDefault
            print 'im array shape =', self.im_array_whole.shape[1]
        ...
        try:
            ...
            pupil_info = self._find_pupil( self.Nrays_quadPupil, StartPupiCoord, StartPupiRadius, FlagForceGuessPupCntr, self.FlagPlotPupil, self.FlagPrintPupil )
            ...
            if self.FlagCompAltGuesses:
                if size(StartPupiCoord) != 0:
                    try:
                        # Next, try to recompute the intial guess, searching an image patch centered on the previous guess
                        if self.FlagPlotFinal: print '------->>> Local search (threshold) on an image patch to find an intial guess for the Pupil'
                        ...
                        self.CR_cntr = cr_info[0]
                        # Finally, start fresh and look for an initial guess over the whole image
                        if self.FlagPlotFinal: print '------->>> Search (threshold) the whole image to find an intial guess for the Pupil'
                        StartPupiCoord = []
    #                     self.ThPup = 0.20 # make it fail
                ...
                # The initial guess based on the Th was wrong ... try other spots 
                    ...
                    Pup_cntr, iPup, jPup = self._first_guess_pupil_centr( FlagPlot )
                    # NOTE: spedwise, creating and then permuting a list is quicker than creating and then permuting an array or using the "permutation" function
                    I = range(len(iPup)); random.shuffle(I)  
    #                 iPup[I[0]] = 190.45
    #                 jPup[I[0]] = 221.12
                    k = 0
                    MaxCount = 10
                    ...
                    while FlagIterate and k < MaxCount:
                        ...
                        k = k+1
                        try:
                            if self.FlagPlotFinal: print '--->>> Attempt #', k, ': try as a new pupil center guess: ', StartPupiCoord
                            if FlagPlot:
                                # Plot image
                                figure()
                                imshow(self.im_array_whole)
                                ...
                                # Plot pupil
                                plot( jPup, iPup, 'g.' )
                                ho = plot( array([StartPupiCoord[0]]), array([StartPupiCoord[1]]), '+b' )
                                setp( ho, 'markersize', 10, 'markeredgewidth', 2 )
                                setp(gca(), 'xlim', [1, self.im_array_whole.shape[1]], 'ylim', [self.im_array_whole.shape[0], 1])
                            pupil_info = self._find_pupil( self.Nrays_quadPupil, StartPupiCoord, StartPupiRadius, FlagForceGuessPupCntr, self.FlagPlotPupil, self.FlagPrintPupil )
                        except:
        ...
        features = { 'pupil_position' : pupil_info[0], 'pupil_radius' : 2*pupil_info[1], 'pupil_radius_CV' : pupil_info[2], 'cr_position' : cr_info[0], 'cr_radius' : 2*cr_info[1], 'cr_radius_CV' : cr_info[2] }
    ...
    def _get_im_array_nrows(self):
        if(self.im_array == None or self.im_array.shape == None or len(self.im_array.shape) < 1):
            return 0
        
        return self.im_array.shape[0]

    def _get_im_array_ncols(self):
        if(self.im_array == None or self.im_array.shape == None or len(self.im_array.shape) < 2):
            return 0
...
#         self.Pup_radius = 0
    ...
    def _first_guess_pupil_center( self, FlagPlot = False, Frame_cntr = None, N_rows_crop = None, N_cols_crop = None ):

        if FlagPlot: print '\nGuess PUPIL center: INITIAL N_rows_crop =', N_rows_crop, '; N_cols_crop =', N_cols_crop
        ...
        if Frame_cntr is None:
            ...
            self.im_array = self.im_array_whole[ Frame_cntr[1]-N_rows_crop :  Frame_cntr[1]+N_rows_crop+1,  Frame_cntr[0]-N_cols_crop :  Frame_cntr[0]+N_cols_crop+1 ]

        if FlagPlot:
            print 'Guess PUPIL center: N_rows_crop =', N_rows_crop, '; N_cols_crop', N_cols_crop
            if Frame_cntr is not None:
                print 'Initial guess =', Frame_cntr
                print 'Cropped rows =', Frame_cntr[1]-N_rows_crop, ',', Frame_cntr[1]+N_rows_crop+1
                print 'Cropped cols =', Frame_cntr[0]-N_cols_crop, ',', Frame_cntr[0]+N_cols_crop+1

        # Find pupil
        (iPup, jPup) = where(self.im_array < self.im_array.min() + self.ThPup * (self.im_array.max()-self.im_array.min()))
        iPup = double(iPup)
        jPup = double(jPup)
    #     Pup_cntr = [jPup.mean(), iPup.mean()]
        ...
        if FlagPlot:
            ...
            setp( ho, 'markersize', 10, 'markeredgewidth', 2 )
            setp(gca(), 'xlim', [1, self.im_array.shape[1]], 'ylim', [self.im_array.shape[0], 1])
    ...
    def _first_guess_CR_center( self, FlagPlot = False, Frame_cntr = None, N_rows_crop = None, N_cols_crop = None ):
    ...
    def _compute_image_gradient( self, Frame_cntr, N_rows_crop, N_cols_crop, FlagPlot=False ):

        if FlagPlot: print '\nGradient patch: INITIAL N_rows_crop =', N_rows_crop, '; N_cols_crop =', N_cols_crop
        ...
        if FlagPadVsCrop:
            ...
            if Frame_cntr[1]+N_rows_crop+1 > self.im_array_whole.shape[0]:
                N_rows_crop_dx = self.im_array_whole.shape[0] - Frame_cntr[1] - 1
            else:
            ...
            if Frame_cntr[0]+N_cols_crop+1 > self.im_array_whole.shape[1]:
                N_cols_crop_bot = self.im_array_whole.shape[1] - Frame_cntr[0] - 1
            else:
            ...
            edge_array = array([]);
            if Frame_cntr[1]-N_rows_crop < 0:
                edge_array = hstack( (edge_array, self.im_array_whole[ Frame_cntr[1]-N_rows_crop_sx, Frame_cntr[0]-N_cols_crop_top : Frame_cntr[0]+N_cols_crop_bot+1 ]) )
            if Frame_cntr[1]+N_rows_crop+1 > self.im_array_whole.shape[0]:
                edge_array = hstack( (edge_array, self.im_array_whole[ Frame_cntr[1]+N_rows_crop_dx, Frame_cntr[0]-N_cols_crop_top : Frame_cntr[0]+N_cols_crop_bot+1 ]) )
            if Frame_cntr[0]-N_cols_crop < 0:
                edge_array = hstack( (edge_array, self.im_array_whole[ Frame_cntr[1]-N_rows_crop_sx : Frame_cntr[1]+N_rows_crop_dx+1, Frame_cntr[0]-N_cols_crop_top ]) )
            if Frame_cntr[0]+N_cols_crop+1 > self.im_array_whole.shape[1]:
                edge_array = hstack( (edge_array, self.im_array_whole[ Frame_cntr[1]-N_rows_crop_sx : Frame_cntr[1]+N_rows_crop_dx+1, Frame_cntr[0]+N_cols_crop_bot ]) )
            
            if edge_array.shape[0] > 0:
                pad_array = edge_array.mean() * ones( (2*N_rows_crop+1, 2*N_cols_crop+1) )
            else:
            ...
            im_array = self.im_array_whole[ Frame_cntr[1]-N_rows_crop_sx : Frame_cntr[1]+N_rows_crop_dx+1, Frame_cntr[0]-N_cols_crop_top : Frame_cntr[0]+N_cols_crop_bot+1 ]
            pad_array[ N_rows_crop-N_rows_crop_sx:N_rows_crop-N_rows_crop_sx+N_rows_crop_sx+N_rows_crop_dx+1, N_cols_crop-N_cols_crop_top:N_cols_crop-N_cols_crop_top+N_cols_crop_top+N_cols_crop_bot+1 ] = im_array
        ...
        if FlagPlot:
            print 'Gradient patch: N_rows_crop =', N_rows_crop, '; N_cols_crop =', N_cols_crop        
            print 'Initial guess =', Frame_cntr
            print 'Cropped rows =', Frame_cntr[1]-N_rows_crop, ',', Frame_cntr[1]+N_rows_crop+1
            print 'Cropped cols =', Frame_cntr[0]-N_cols_crop, ',', Frame_cntr[0]+N_cols_crop+1
        ...
        if EdgeDetectAlg == 'sobel3x3separable':
            use_sse3 = 0
            (self.im_grad_mag, self.im_grad_x, self.im_grad_y) = sobel3x3_separable(self.im_array, use_sse3)
        elif EdgeDetectAlg == 'sobel3x3': # unfortunately is too slow (takes 2 times more than regular gradient)
            sobel_y = array([[-1,-2,-1],[0,0,0],[1,2,1]])
            sobel_x = array([[-1,0,1],[-2,0,2],[-1,0,1]])
            self.im_grad_y = signal.correlate2d( self.im_array, sobel_y, mode='same', boundary='symm' )
            self.im_grad_x = signal.correlate2d( self.im_array, sobel_x, mode='same', boundary='symm' )
...
#         print 'min in x grad=', self.im_grad_x.min()
        ...
        if EdgeDetectAlg != 'sobel3x3separable':
            ...
            if FlagApprGradMag:
                ...
                self.im_grad_mag = sqrt( pow(self.im_grad_y,2) + pow(self.im_grad_x,2) )
    ...
    # ==================================== function: _findPupil ========================================
    def _find_pupil( self, Nrays_quad = 2, StartPupiCoord = [], StartPupiRadius = None, FlagForceGuessPupCntr = False, FlagPlot = False, FlagPrint = False ):
        ...
        if StartPupiRadius is not None:
            ...
            N_rows_crop = N_cols_crop = int( self.mFact_PupRadius2GradImgPatch * StartPupiRadius )
        else:
        ...
        if size(StartPupiCoord) == 0:
            ...
            # Search for a better guess in a patch of image centered on the Pupil coordinated passed as arguments
                ...
                Pup_cntr, iPup, jPup = self._first_guess_pupil_center( FlagPlot, StartPupiCoord, int( self.mFatc_PupGradImgPatch2ThImgPatch * N_rows_crop ), int( self.mFatc_PupGradImgPatch2ThImgPatch * N_cols_crop ) )
        ...
        N_rows_crop, N_cols_crop = self._compute_image_gradient( Pup_cntr, N_rows_crop, N_cols_crop, FlagPlot )
        ...
        rays_end = hstack( (x[:,x.shape[1]-1].reshape(-1,1), y[:,y.shape[1]-1].reshape(-1,1) ) )
        ...
        CurrTh = 1000
        i_peak_right, i_peak_left, CurrTh = self.SB.find_edge_along_rays_from_center( self.im_grad_mag, x, y, CurrTh, FlagPlot, FlagPrint )
        if FlagPrint: print 'i peak finali: \nleft =\n', i_peak_left, '\nright =\n', i_peak_right
        ...
        if FlagPrint: print 'i peak finali (after removal of NaNs): \nleft =\n', i_peak_left, '\nright =\n', i_peak_right
        if not Iok_right and not Iok_left:
            raise Exception, "No ray was found that intersected the pupil boundary in 2 points!"

        # Final vectors with coordinates of boundary points
        x_bound = concatenate( (x[arange(x.shape[0])[Iok_right],i_peak_right[Iok_right]], x[arange(x.shape[0])[Iok_left],i_peak_left[Iok_left]]), axis = 0 ).astype(float)
        y_bound = concatenate( (y[arange(x.shape[0])[Iok_right],i_peak_right[Iok_right]], y[arange(x.shape[0])[Iok_left],i_peak_left[Iok_left]]), axis = 0 ).astype(float)   
    #     print 'run time first starburst =', runtime_starburst1
        if FlagPrint: print 'x on boundary = \n', x_bound, '\ny on boundary =\n', y_bound

        # Eliminate intersections that are too close to the CR (because the CR may overlap with the pupil)
        d2CR = sqrt((x_bound - self.CR_cntr[0])**2 + (y_bound - self.CR_cntr[1])**2)
        Ibad = where( d2CR < self.mfact_CR_radius*self.CR_radius )[0]
        if FlagPrint:
            print 'distance from CR =', d2CR
            print 'Intersections removed because too close to the CR:  x =', x_bound[Ibad], '\ty =', y_bound[Ibad]
        if FlagPlot:
        ...
        if FlagPrint: print 'After removal of CR boundaries: x on boundary = \n', x_bound, '\ny on boundary =\n', y_bound
        if x_bound.shape[0] == 0:
            raise Exception, "No ray was found whose intersections with the pupil were NOT too close to the CR!"

        # Save final boundary points
        boundPoints = hstack( (x_bound.reshape(-1,1), y_bound.reshape(-1,1)) ) 

        # ========= Least square minimization to fit a circle trough the interseciton of the rays with the pupil edges =========
        # Initial guess of circle parameters
        a0 = -2 * Pup_cntr[0]
        b0 = -2 * Pup_cntr[1]
        c0 = Pup_cntr[0]**2 + Pup_cntr[1]**2 - (x_bound - Pup_cntr[0]).mean()**2
        p0 = [a0, b0, c0]
        if FlagPrint: print 'p0 = ', p0
        # Fit circle to the data points
        p = optimize.leastsq( self._residuals_circle, p0, args=(y_bound, x_bound))
        if FlagPrint: print 'circle fit parameters = ', p
        a,b,c = p[0] 
        # Calculate the location of center and radius
        Pup_cntr_fit = [-a/2, -b/2]
        Pup_radius_fit = sqrt( (Pup_cntr_fit[0]**2+Pup_cntr_fit[1]**2) - c )
        if FlagPrint:
        ...
        if FlagPlot:
            ...
            ho = plot( array([Pup_cntr_fit[0]]), array([Pup_cntr_fit[1]]), '+r' )
            setp( ho, 'markersize', 10, 'markeredgewidth', 2 )
            plot( array([Pup_cntr_fit[0]-Pup_radius_fit, Pup_cntr_fit[0]+Pup_radius_fit]), array([Pup_cntr_fit[1], Pup_cntr_fit[1]]), '-' )       
        ...
        radiuses = sqrt( (x_bound - Pup_cntr_fit[0])**2 +  (y_bound - Pup_cntr_fit[1])**2 )
        Pup_radius_mean = radiuses.mean()
        Pup_radius_std = radiuses.std()
        
        # Find outaliers
        Ioutlay =  where( (radiuses > Pup_radius_mean+1.0*Pup_radius_std) | (radiuses < Pup_radius_mean-1.0*Pup_radius_std) )[0]
        if FlagPrint:
            ...
            print 'Intersections removed because they are outlayers:  x =', x_bound[Ioutlay], '\ty =', y_bound[Ioutlay]
        if FlagPlot:
            # Show the intersection points that were removed because too close to the CR
        ...
        if Ioutlay.shape[0] > 0:
            ...
            # Save final boundary points
            boundPoints = hstack( (x_bound.reshape(-1,1), y_bound.reshape(-1,1)) ) 
            if FlagPrint: print 'After removal of outlayers intersection points: x on boundary = \n', x_bound, '\ny on boundary =\n', y_bound
            if x_bound.shape[0] == 0:
                raise Exception, "No ray was found whose intersections were NOT OUTLIERS!"
        ...
        if FlagPlot: self.FlagPlotFinal = True
        ...
        rays_start = ones( (rays_start.shape[0],1) ) * Pup_cntr_whole_img + rays_start - ones( (rays_start.shape[0],1) ) * Pup_cntr
        rays_end = ones( (rays_end.shape[0],1) ) * Pup_cntr_whole_img + rays_end - ones( (rays_end.shape[0],1) ) * Pup_cntr
        boundPoints = ones( (boundPoints.shape[0],1) ) * Pup_cntr_whole_img + boundPoints - ones( (boundPoints.shape[0],1) ) * Pup_cntr
        ...
        if self.FlagPlotFinal:
            ...
            imshow(self.im_array_whole)
            hold(1)
            ho = plot( array([Pup_cntr_whole_img[0]]), array([Pup_cntr_whole_img[1]]), '+b' )
            setp( ho, 'markersize', 10, 'markeredgewidth', 2 )
        ...
        return self.Pup_cntr, self.Pup_radius, Pup_radius_CV, rays_start, rays_end, boundPoints
#             return Pup_cntr_new, Pup_cntr_new2, Pup_cntr_new3, runtime_starburst1, runtime_starburst2, x_min_lstsq1, runtime_lstsq1, x_min_lstsq2, runtime_lstsq2, Nrays
    ...
    # ==================================== function: _findCR ========================================
    def _find_CR( self, Nrays_quad = 2, StartCRCoord = [], StartCRRadius = None, FlagForceGuessCRCntr = False, FlagPlot = False, FlagPrint = False ):
        """ Extract corneal reflection (CR) and pupil center from an eye image.
        ...
        if StartCRRadius is not None:
            ...
            N_rows_crop = N_cols_crop = int( self.mFact_CRRadius2GradImgPatch * StartCRRadius )
        else:
        ...
        if size(StartCRCoord) == 0:
            ...
            # Search for a better guess in a patch of image centered on the CR coordinated passed as arguments
                ...
                CR_cntr, iCR, jCR = self._first_guess_CR_center( FlagPlot, StartCRCoord, int( self.mFatc_CRGradImgPatch2ThImgPatch * N_rows_crop ), int( self.mFatc_CRGradImgPatch2ThImgPatch * N_cols_crop )  )
        ...
        if self.im_grad_x.shape[0] == 0: 
            ...
            N_rows_crop, N_cols_crop = self._compute_image_gradient( CR_cntr, N_rows_crop, N_cols_crop, FlagPlot )
        ...
        if FlagPrint: print 'i peak finali (after removal of NaNs): \nleft =\n', i_peak_left, '\nright =\n', i_peak_right
        if not Iok_right and not Iok_left:
            raise Exception, "No ray was found that intersected the CR boundary in 2 points!"
        ...
        a0 = -2 * CR_cntr[0]
        b0 = -2 * CR_cntr[1]
        c0 = CR_cntr[0]**2 + CR_cntr[1]**2 - (x_bound - CR_cntr[0]).mean()**2
        p0 = [a0, b0, c0]
        if FlagPrint: print 'p0 = ', p0
        # Fit circle to the data points
        p = optimize.leastsq( self._residuals_circle, p0, args=(y_bound, x_bound))
        ...
        if FlagPlot:
            ...
            setp( ho, 'markersize', 10, 'markeredgewidth', 2 )
            plot( array([CR_cntr_fit[0]-CR_radius_fit, CR_cntr_fit[0]+CR_radius_fit]), array([CR_cntr_fit[1], CR_cntr_fit[1]]), '-' )        
        
        # Compute radius of the CR
        radiuses = sqrt( (x_bound - CR_cntr_fit[0])**2 +  (y_bound - CR_cntr_fit[1])**2 )        
        self.CR_radius = CR_radius_fit
        # Test that the CR is round enough (based on std of radiuses)
        CR_radius_CV = radiuses.std() / self.CR_radius
        if FlagPrint:
        ...
        # Do the same for the rays and boundary points coordinates
        rays_start = ones( (rays_start.shape[0],1) ) * CR_cntr_whole_img + rays_start - ones( (rays_start.shape[0],1) ) * CR_cntr
        rays_end = ones( (rays_end.shape[0],1) ) * CR_cntr_whole_img + rays_end - ones( (rays_end.shape[0],1) ) * CR_cntr
        boundPoints = ones( (boundPoints.shape[0],1) ) * CR_cntr_whole_img + boundPoints - ones( (boundPoints.shape[0],1) ) * CR_cntr
        ...
        if self.FlagPlotFinal:
            ...
            imshow(self.im_array_whole)
            hold(1)
            ho = plot( array([CR_cntr_whole_img[0]]), array([CR_cntr_whole_img[1]]), '+b' )
            setp( ho, 'markersize', 10, 'markeredgewidth', 2 )
        ...
        return self.CR_cntr, self.CR_radius, CR_radius_CV, rays_start, rays_end, boundPoints
...
class StarBurst:
    ...
    def find_edge_along_rays_from_center( self, im_grad_mag, x, y, CurrTh = 1000, FlagPlot = False, FlagPrint = False ):
        pass

    def find_edge_along_rays_to_right( self, im_grad_mag, x, y, i_shift, CurrTh = 1000, FlagPlot = False, FlagPrint = False ):
        pass

    def find_edge_along_rays_to_left( self, im_grad_mag, x, y, i_shift, CurrTh = 1000, FlagPlot = False, FlagPrint = False ):
        pass
...
class StarBurstLoop(StarBurst):
    ...
    def find_edge_along_rays_from_center( self, im_grad_mag, x, y, CurrTh = 1000, FlagPlot = False, FlagPrint = False ):
        ...
        for i in xrange(grad_mag_star.shape[0]):
            ...
            if FlagPrint: print 'Iok_ctr = ', Iok_ctr
        ...
        if FlagPlot:
            ...
            figure();
            ray_ax = ones( (grad_mag_star.shape[0],1) ) * arange( grad_mag_star.shape[1] )
            ...
            plot( ray_ax[ir,ic].transpose(), grad_mag_star[ir,ic].transpose(), 'o' )
            # Plot pupil edges
            for i in xrange(grad_mag_star.shape[0]):
                ho = plot( [ray_ax[i,i_peak_right[i]]], [grad_mag_star[i,i_peak_right[i]]], '+r' )
                setp( ho, 'markersize', 10, 'markeredgewidth', 2 )
                ho = plot( [ray_ax[i,i_peak_left[i]]], [grad_mag_star[i,i_peak_left[i]]], '+g' )
                setp( ho, 'markersize', 10, 'markeredgewidth', 2 )
    ...
    def find_edge_along_rays_to_right( self, im_grad_mag, x, y, i_shift, CurrTh = 1000, FlagPlot = False, FlagPrint = False ):
        ...
        grad_mag_star = im_grad_mag[y,x]
        ray_ax = ones( (grad_mag_star.shape[0],1) ) * arange( grad_mag_star.shape[1] )
        if FlagPrint: print '\nIn FindPupEdgeAlongRays_dx: grad_mag_star.shape =', grad_mag_star.shape
        ...
        for i in xrange(grad_mag_star.shape[0]):
            Iok = where( grad_mag_star[i,:] > ThGrad ) 
            if FlagPrint: print 'iok =', Iok
            Ith.append( Iok[0] )
            Iok_ctr = Iok[0]-i_shift
            IIok_ctr_p = where( Iok_ctr > 0 ) # points > Th to the right of i_shift
            if FlagPrint: print 'II edges = ', IIok_ctr_p, 'truth value =', IIok_ctr_p[0].any()   
        ...
        if FlagPlot:
            ...
            Col = ['b', 'g', 'r', 'c', 'm', 'k', 'y', 'b', 'g', 'r', 'c', 'm', 'k', 'y' ]
            plot( ray_ax.transpose(), grad_mag_star.transpose(), '.-' )
            for i in arange(grad_mag_star.shape[0])[Iok_right]:
                plot( [0, grad_mag_star.shape[1]], [ThGrad, ThGrad], '--'+Col[i] )
                plot( ray_ax[i,Ith[i]], grad_mag_star[i,Ith[i]], 'o'+Col[i] )
                ho = plot( [ray_ax[i,i_peak_right[i]]], [grad_mag_star[i,i_peak_right[i]]], '+k' )
                setp( ho, 'markersize', 10, 'markeredgewidth', 2 )
    ...
    def find_edge_along_rays_to_left( self, im_grad_mag, x, y, i_shift, CurrTh = 1000, FlagPlot = False, FlagPrint = False ):
        ...
        grad_mag_star = im_grad_mag[y,x]
        ray_ax = ones( (grad_mag_star.shape[0],1) ) * arange( grad_mag_star.shape[1] )
        if FlagPrint: print '\nIn FindPupEdgeAlongRays_sx: grad_mag_star.shape =', grad_mag_star.shape
    ...
    #         N_IIok_ctr_m = size(IIok_ctr_m)
            if FlagPrint: print 'II edges = ', IIok_ctr_m, 'truth value =', IIok_ctr_m[0].any()
            # Check if an edge was found (i.e., IIok_ctr_m is not empty)
        ...
        if FlagPlot:
            ...
            for i in arange(grad_mag_star.shape[0])[Iok_left]:
                plot( [0, grad_mag_star.shape[1]], [ThGrad, ThGrad], '--'+Col[i] )
                plot( ray_ax[i,Ith[i]], grad_mag_star[i,Ith[i]], 'o'+Col[i] )
                ho = plot( [ray_ax[i,i_peak_left[i]]], [grad_mag_star[i,i_peak_left[i]]], '+k' )
                setp( ho, 'markersize', 10, 'markeredgewidth', 2 )
...
class StarBurstVectorized(StarBurst):
    ...
    def _get_image_values(self, im, x, y):
        return self._get_image_values_nearest(im,x,y)
    ...
    def find_edge_along_rays_from_center( self, im_grad_mag, x, y, CurrTh = 1000, FlagPlot = False, FlagPrint = False ):
        ...
        if ir.shape[0] == 0 or ic.shape[0] == 0:
            raise Exception, 'In shoot rays from center: No intersection with Pupil/CR boundary was found!'

        # Find intersections of rays wit the pupil boundary
        x_idx = arange(0, grad_mag_star.shape[1])
        ColIdxIn_grad_star = (ones((grad_mag_star.shape[0],1)) * x_idx).astype(int) 
        if FlagPrint: print '\n gradient =', grad_mag_star; print '\n indexes =', ColIdxIn_grad_star
        # Points > Th to the RIGHT of pupil center
        (Iok, Jok) = where( (grad_mag_star > ThGrad) & ((ColIdxIn_grad_star-grad_mag_star.shape[1]/2) > 0) )
        if FlagPrint: print '\nIok=', Iok; print '\nJok=', Jok
        if Iok.shape[0] == 0:
            raise Exception, 'No RIGHT intersection with Pupil/CR boundary was found!'
        Istart_ray = (concatenate( ([Iok[0]+1], Iok[:len(Iok)-1] ) ) - Iok).nonzero()
        Idx_ray = Iok[Istart_ray]
        if FlagPrint: print 'Idx_ray=', Idx_ray; print 'Istart_ray=', Istart_ray; print 'Indexes peaks right =', Jok[Istart_ray]
        ...
        if FlagPrint: print 'i_peak finale right = ', i_peak_right

        # Points > Th to the LEFT of pupil center
        (Iok, Jok) = where( (grad_mag_star > ThGrad) & ((ColIdxIn_grad_star-grad_mag_star.shape[1]/2) < 0) )
        if Iok.shape[0] == 0:
            raise Exception, 'No LEFT intersection with Pupil/CR boundary was found!'
        Iok = Iok[len(Iok)::-1]
        Jok = Jok[len(Jok)::-1]
        if FlagPrint: print '\n gradient =', grad_mag_star; print '\n indexes =', ColIdxIn_grad_star
        Istart_ray = (concatenate( ([Iok[0]+1], Iok[:len(Iok)-1] ) ) - Iok).nonzero()
        Idx_ray = Iok[Istart_ray]
        if FlagPrint: print 'Idx_ray=', Idx_ray; print 'Indexes peaks left =', Jok[Istart_ray]
        i_peak_left = zeros((grad_mag_star.shape[0]))
        i_peak_left[Idx_ray] = Jok[Istart_ray]
        Idx_ray_all = arange(0,grad_mag_star.shape[0])
        Inan = setdiff1d(Idx_ray_all,Idx_ray)
        i_peak_left[Inan] = NaN    
        if FlagPrint: print 'i_peak finale left = ', i_peak_left
    ...
    def find_edge_along_rays_to_right( self, im_grad_mag, x, y, i_shift, CurrTh = 1000, FlagPlot = False, FlagPrint = False ):
        ...
        (ir, ic) = where( grad_mag_star > ThGrad )
        if ir.shape[0] == 0 or ic.shape[0] == 0:
            raise Exception, 'In shoot rays to dx (1): No intersection with Pupil/CR boundary was found!'
        ...
        if FlagPrint: print '\n gradient =', grad_mag_star; print '\n indexes =', ColIdxIn_grad_star
        # Points > Th to the RIGHT of pupil center
        (Iok, Jok) = where( (grad_mag_star > ThGrad) & ((ColIdxIn_grad_star-i_shift) > 0) )
        if FlagPrint: print '\nIok=', Iok; print '\nJok=', Jok
        if Iok.shape[0] == 0:
            raise Exception, 'In shoot rays to dx (2): No intersection with Pupil/CR boundary was found!'
        Istart_ray = (concatenate( ([Iok[0]+1], Iok[:len(Iok)-1] ) ) - Iok).nonzero()
        Idx_ray = Iok[Istart_ray]
        if FlagPrint: print 'Idx_ray=', Idx_ray; print 'Istart_ray=', Istart_ray; print 'Indexes peaks right =', Jok[Istart_ray]
        i_peak_right = zeros((grad_mag_star.shape[0]))
...
#                 plot( ray_ax[i,Ith], grad_mag_star[i,Ith], 'ok' )
    ...
    def find_edge_along_rays_to_left( self, im_grad_mag, x, y, i_shift, CurrTh = 1000, FlagPlot = False, FlagPrint = False ):
        ...
        (ir, ic) = where( grad_mag_star > ThGrad )
        if ir.shape[0] == 0 or ic.shape[0] == 0:
            raise Exception, 'In shoot rays to sx (1):No intersection with Pupil/CR boundary was found!'
        ...
        if FlagPrint: print '\n gradient =', grad_mag_star; print '\n indexes =', ColIdxIn_grad_star
        # Points > Th to the LEFT of pupil center
        (Iok, Jok) = where( (grad_mag_star > ThGrad) & ((ColIdxIn_grad_star-(grad_mag_star.shape[1] - i_shift)) < 0) )
        if Iok.shape[0] == 0:
            raise Exception, 'In shoot rays to sx (2): No intersection with Pupil/CR boundary was found!'
        Iok = Iok[len(Iok)::-1]
        Jok = Jok[len(Jok)::-1]
        
        if FlagPrint: print '\n gradient =', grad_mag_star; print '\n indexes =', ColIdxIn_grad_star
        Istart_ray = (concatenate( ([Iok[0]+1], Iok[:len(Iok)-1] ) ) - Iok).nonzero()
        t_ray = (concatenate( ([Iok[0]+1], Iok[:len(Iok)-1] ) ) - Iok).nonzero()
        Idx_ray = Iok[Istart_ray]
...
test_image_name = "Snapshot_test.bmp"

test_image = asarray(PIL.Image.open(test_image_name))
test_image = mean(test_image,2)

radial_ff = FastRadialFeatureFinder()
starburst_ff = StarBurstEyeFeatureFinder()
...
print cr_position
plot([cr_position[1]], [cr_position[0]], 'kx')
...
for i in range(0, len(cr_ray_start)):
    plot([cr_ray_start[i][1], cr_ray_end[i][1]], [cr_ray_start[i][0], cr_ray_end[i][0]])
for b in cr_bounds:
    plot([b[1]], [b[0]], "rx")
    pass
axis((cr_position[1]+pw, cr_position[0]+pw, cr_position[1]-pw, cr_position[0]-pw))
