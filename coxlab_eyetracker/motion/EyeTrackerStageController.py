...
class EyeTrackerStageController:
    ...
    def setup(self):
        ...
        self.controller.send("1HN%d,%d,%d" % (self.x_axis,self.y_axis,self.r_axis),1)
    ...
    def precompute_return_motion(self):
        ...
        return lambda: self.move_composite_absolute((self.x_axis,self.y_axis,self.r_axis), (x,y,r))
    ...
    def precompute_composite_rotation_relative(self, d, r_rel):
        ...
        r_abs = r0_abs + r_rel
        x_abs = x0_abs - d * sin(r0_abs*math.pi/180) + d * cos(r0_abs*math.pi/180) * tan(r_abs*math.pi/180)
        ...
        # Move the axes
        return lambda: self.controller.move_composite_absolute( (self.x_axis, self.r_axis), (x_abs,r_abs) ), d_new
    ...
    def composite_rotation_absolute(self, d, r):
        r = float(r)
        d = float(d)
        x = sqrt( -(d**2 * sin(r*math.pi/180)**2) / (sin(r * math.pi / 180)**2 - 1) )
    
        if(r < 0):
            x = -x
        
        self.controller.move_composite_absolute( (self.x_axis, self.r_axis), (x,r) )
    ...
    def composite_rotation_relative(self, d, r_rel):
        ...
        # Move the axes
        self.controller.move_composite_absolute( (self.x_axis, self.r_axis), (x_abs,r_abs) )
        
        # Compute the new distance of the rotation center from the camera (following the current relative rotation)
        d_new = d * cos(r0_abs*math.pi/180) / cos(r_abs*math.pi/180)
        
        # return a function call to undo this movement
        return d_new, lambda: self.controller.move_composite_absolute( (self.x_axis, self.r_axis), (x0_abs,r0_abs) )
    ...
    def composite_rotation_relative_old(self, d, r):
        ...
        Dr = r
        Dx = sqrt( -(d**2 * sin(r*math.pi/180)**2) / (sin(r * math.pi / 180)**2 - 1) )
        ...
        self.controller.move_composite_relative( (self.x_axis, self.r_axis), (Dx, Dr) )
        
        # return a function call to undo this movement
        return lambda: self.controller.move_composite_relative( (self.x_axis, self.r_axis), (-Dx, -Dr) )
