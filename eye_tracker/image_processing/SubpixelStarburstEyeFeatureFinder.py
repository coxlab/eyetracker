...
#            pupil_position, pupil_radius, pupil_err = self._fit_points(pupil_boundaries)
    ...
    def _find_ray_boundaries_woven(self, im, seed_point, zero_referenced_rays, cutoff_index, threshold, **kwargs):
        ...
        inline(code, ['vals', 'vals_slope', 'rays_x', 'rays_y', 'cutoff_index', 'threshold', 'n_returned_points', 'returned_points'])
        boundary_points = []
                
        for p in range(0, returned_points.shape[0]):
            if(returned_points[p,0] != -1.0):
        ...
        for i in range(0,x.shape[0]):
            for j in range(0, x.shape[1]):
                if(floor_x[i,j] < 0 or floor_y[i,j] < 0 or \
                    ceil_x[i,j] > im.shape[0] or ceil_y[i,j] > im.shape[1]):
                    vals[i,j] = nan
                    continue
    ...
    def _get_image_values_interp_faster(self, im, x,y):
        ...
        vals = x_frac * y_frac * a + (1-x_frac) * y_frac * b + \
                      x_frac * (1 - y_frac) * c + (1-x_frac)*(1-y_frac)*d
    ...
    def _fit_circle_to_points_lstsq_ransac(self, points):
        ...
        center_fit, radius_fit, best_err = self._fit_circle_to_points_lstsq(points)
        ...
        while iter < max_iter:
            ...
            maybe_center, maybe_radius, maybe_err = self._fit_circle_to_points_lstsq(maybe_inliers)
            ...
            if(len(maybe_inliers) > good_fit_consensus):
                candidate_center, candidate_radius, candidate_err = self._fit_circle_to_points_lstsq(array(maybe_inliers))
