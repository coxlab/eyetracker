...
class NaiveSeparableConvolutionKernel (MetaKernel):
    ...
    def __call__(self, input_im, row_kernel, col_kernel, result=None, input_shape=None, row_shape=None, col_shape=None, **kwargs):
        ...
        t = Timer()
        try:
            exec_evt = prg.separable_convolution_row(self.queue, input_im.shape, intermediate_dev, input_dev, numpy.uint32(input_im.shape[1]), numpy.uint32(input_im.shape[0]), row_dev, numpy.uint32(row_dev.shape[0]))
        ...
        t = Timer()
        try:
            exec_evt = prg.separable_convolution_col(self.queue, input_im.shape, result_dev, intermediate_dev, numpy.uint32(input_im.shape[1]), numpy.uint32(input_im.shape[0]), col_dev, numpy.uint32(col_dev.shape[0]))
        except Exception as e:
            print(input_im.shape)
            print(result_dev)
            print(intermediate_dev)
...
class LocalMemorySeparableConvolutionKernel (MetaKernel):
    ...
    def __call__(self, input_im, row_kernel, col_kernel, result=None, input_shape=None, row_shape=None, col_shape=None, **kwargs):
        ...
        col_tile_height = 48
        col_hstride = 8
        assert numpy.mod(row_dev.shape[0],2) == 1, "Kernels must be of odd width"
        ...
        assert numpy.mod(col_dev.shape[0],2) == 1, "Kernels must be of odd width"
    ...
    def __call__(self, image, radii, alpha, sobel_triplet=None, **kwargs):
        ...
        for r in range(0, len(radii)):
            ...
            def run_inline(tile, O,M,mag,posx,posy,negx,negy,kappa,F,alpha,n_tiles):	
                inline(code, ['O','M', 'mag','posx','posy','negx','negy','kappa', 'F', 'alpha', 'n_tiles', 'tile'], verbose=0)#, type_converters=converters.blitz)
            ...
            if(multithreaded_weave):
                ...
                n_tiles = 1
                tile = 0
                inline(code, ['O','M', 'mag','posx','posy','negx','negy','kappa', 'F', 'alpha', 'n_tiles', 'tile'], verbose=0)
